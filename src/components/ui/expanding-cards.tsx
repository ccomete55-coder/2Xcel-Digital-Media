...
interface ExpandingCardsProps extends React.HTMLAttributes<HTMLUListElement> {
  ...
  onToggleSelect?: (item: CardItem) => void;
}

export const ExpandingCards = React.forwardRef<
  HTMLUListElement,
  ExpandingCardsProps
>(({ className, items, defaultActiveIndex = 0, onToggleSelect, ...props }, ref) => {
      ...
      {items.map((item, index) => (
        <li
          ...
          className={cn(
            ...
            "md:min-w-[80px]",
            "min-h-0 min-w-0",
            item.selected && "ring-2 ring-brand-orange ring-offset-2 ring-offset-black",
          )}
